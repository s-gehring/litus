...
async function retryCleanup(dir: string, throwOnFailure = false): Promise<void> {
...
export interface TempDirOptions {
	/** If true, throw when cleanup fails after all retries (default: false — warns only) */
...
export async function withTempDir(
	...
	try {
		...
		await retryCleanup(dir, options?.throwOnCleanupFailure);
	}
}
